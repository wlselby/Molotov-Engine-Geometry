...
r_chamber = 0.0127;          % Combustion chamber radius [m]
...
Pc        = 344737.9;        % Chamber pressure [Pa] (~400 psi)
...
%% Helper Geometry Functions Exporting CSV Files for Onshape
function generateNozzleContour(r_throat, r_exit)
    theta_deg  = 30;
    alpha_deg  = 15;
    R_arc_conv = 1.5 * r_throat;
    R_arc_div  = 1.5 * r_throat;

    x_throat = (r_exit - r_throat + R_arc_conv*(secd(theta_deg)-1))/tand(theta_deg);
    x_tan1   = x_throat - R_arc_conv * sind(theta_deg);
    y_tan1   = r_throat + R_arc_conv * (1 - cosd(theta_deg));
    
    x_conv = linspace(0, x_tan1, 30);
    y_conv = y_tan1 + (x_tan1 - x_conv) * tand(theta_deg);

    t_arc1 = linspace(-90 - theta_deg, -90, 20);
    x_arc1 = x_throat + R_arc_conv * cosd(t_arc1);
    y_arc1 = (r_throat + R_arc_conv) + R_arc_conv * sind(t_arc1);

    t_arc2 = linspace(-90, -90 + alpha_deg, 20);
    x_arc2 = x_throat + R_arc_div * cosd(t_arc2);
    y_arc2 = (r_throat + R_arc_div) + R_arc_div * sind(t_arc2);

    x_tan2 = x_throat + R_arc_div * sind(alpha_deg);
    y_tan2 = r_throat + R_arc_div * (1 - cosd(alpha_deg));
    L_div  = (r_exit - r_throat + R_arc_div * (secd(alpha_deg) - 1)) / tand(alpha_deg);
    x_exit = x_throat + L_div;

    x_div  = linspace(x_tan2, x_exit, 40);
    y_div  = y_tan2 + (x_div - x_tan2) * tand(alpha_deg);

    X = [x_conv, x_arc1(2:end), x_arc2(2:end), x_div(2:end)]' * 1000; % [mm]
    Y = [y_conv, y_arc1(2:end), y_arc2(2:end), y_div(2:end)]' * 1000; % [mm]
    Z = zeros(size(X));

    writematrix([X, Y, Z], 'nozzle_contour.csv');
    fprintf('Saved: nozzle_contour.csv\n');
end

function generateFullEngineContour(r_chamber, r_throat, r_exit, L_cyl)
    theta_deg  = 30;
    alpha_deg  = 15;
    R_arc_conv = 1.5 * r_throat;
    R_arc_div  = 1.5 * r_throat;

    x_cyl = linspace(0, L_cyl, 40);
    y_cyl = r_chamber * ones(size(x_cyl));
    ...
    x_conv = linspace(L_cyl, x_tan1, 30);
    y_conv = y_tan1 + (x_tan1 - x_conv) * tand(theta_deg);
    ...
    X = [x_cyl, x_conv(2:end), x_arc1(2:end), x_arc2(2:end), x_div(2:end)]' * 1000; % [mm]
    Y = [y_cyl, y_conv(2:end), y_arc1(2:end), y_arc2(2:end), y_div(2:end)]' * 1000; % [mm]
    Z = zeros(size(X));

    writematrix([X, Y, Z], 'full_engine_contour.csv');
    fprintf('Saved: full_engine_contour.csv\n');
end

function generateParabolicEngineContour(r_chamber, r_throat, r_exit, L_cyl, wall_thickness_mm)
    theta_deg  = 30;
    alpha_deg  = 15;
    R_arc_conv = 1.5 * r_throat;
    R_arc_div  = 1.5 * r_throat;
    ...
    X_ID = [x_cyl, x_conv(2:end), x_arc1(2:end), x_arc2(2:end), x_div(2:end)]';
    Y_ID = [y_cyl, y_conv(2:end), y_arc1(2:end), y_arc2(2:end), y_div(2:end)]';

    % Calculate outer wall using normal offset
    dx  = gradient(X_ID);
    dy  = gradient(Y_ID);
    len = hypot(dx, dy);
    nx  = -dy ./ len;
    ...
    wall_m = wall_thickness_mm / 1000;
    X_OD   = X_ID + wall_m * nx;
    Y_OD   = Y_ID + wall_m * ny;

    % Scale to mm
    ID_matrix = [X_ID * 1000, Y_ID * 1000, zeros(size(X_ID))];
    OD_matrix = [X_OD * 1000, Y_OD * 1000, zeros(size(X_OD))];

    writematrix(ID_matrix, 'engine_inner_contour.csv');
    writematrix(OD_matrix, 'engine_outer_contour.csv');
    fprintf('Saved: engine_inner_contour.csv & engine_outer_contour.csv\n');
end